...
def compute_A(A, n1, n2, u):
    ...
    for n in range(1, len(A)):
        A[n,n-1] = np.sqrt(((2.*n)*getK(n-1.))/getK(n)) * A[n,n] * u
...
class Regression:
    ...
    def perform_regression(self, remove_deg=False):
        M = populate_M(self.rVec1D, self.A, self.n1, self.n2, self.N, self.a, self.mu, remove_deg)
...
def main():
    ...
    max_true_deg = 4
    regress_deg = 4
    sh_EGM2008 = SphericalHarmonics('C:\\Users\\John\\Documents\\Research\\ML_Gravity\\GravNN\\Files\\GravityModels\\EGM2008_to2190_TideFree_E.txt', regress_deg)
    ...
    start = time.time()
    coefficients = regressor.perform_regression(remove_deg)
    print(time.time() - start)
    regressor.save('C:\\Users\\John\\Documents\\Research\\ML_Gravity\\GravNN\\Files\\GravityModels\\Regressed\\some.csv')
    #print(coefficients)

    sh_test = SphericalHarmonics('C:\\Users\\John\\Documents\\Research\\ML_Gravity\\GravNN\\Files\\GravityModels\\Regressed\\truth.csv', regress_deg)
    sh_regress = SphericalHarmonics('C:\\Users\\John\\Documents\\Research\\ML_Gravity\\GravNN\\Files\\GravityModels\\Regressed\\some.csv', regress_deg)


    #print(sh_EGM2008.C_lm[:,0])
    print(np.round((sh_regress.C_lm[:,0] - sh_EGM2008.C_lm[:,0][:-2])/sh_EGM2008.C_lm[:,0][:-2]*100))
    print(np.round((sh_regress.C_lm[:,1] - sh_EGM2008.C_lm[:,1][:-2])/sh_EGM2008.C_lm[:,1][:-2]*100))
    print(np.round((sh_regress.C_lm[:,2] - sh_EGM2008.C_lm[:,2][:-2])/sh_EGM2008.C_lm[:,2][:-2]*100))
    print(np.round((sh_regress.C_lm[:,3] - sh_EGM2008.C_lm[:,3][:-2])/sh_EGM2008.C_lm[:,3][:-2]*100))

    errors = np.round((sh_regress.C_lm.reshape((-1)) - sh_EGM2008.C_lm[:-2,:-2].reshape((-1)))/sh_EGM2008.C_lm[:-2,:-2].reshape((-1))*100,1)
    print(errors)
