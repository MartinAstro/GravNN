...
from GravNN.Regression.utils import populate_H_singular, getK, format_coefficients, populate_removed_degrees, save
...
class SGD:
    ...
    def initialize_pines_params(self, degree):
        ...
        for ii in range(0, degree+2):
            ...
            if ii == 0:
                ...
                aBar[ii,ii] = np.sqrt((2*ii+1)*getK(ii)/(2*ii*getK(ii-1)))*aBar[ii-1,ii-1]

            # loop through order
            for jj in range(0, ii+1):
                if ii >= jj + 2:
                    n1[ii,jj] = np.sqrt((2*ii+1)*(2*ii-1)/((ii-jj)*(ii+jj)))
                    n2[ii,jj] = np.sqrt((ii+jj-1)*(2*ii+1)*(ii-jj-1)/((ii+jj)*(ii-jj)*(2*ii-3)))

        for ii in range(0, degree+1):
            for jj in range(0,ii+1):
                if jj < ii:
                    nq1[ii,jj] = np.sqrt((ii-jj)*getK(jj)*(ii+jj+1)/getK(jj+1))
                nq2[ii,jj] = np.sqrt((ii+jj+2)*(ii+jj+1)*(2*ii+1)*getK(jj)/((2*ii+3)*getK(jj+1)))
    ...
    def computePinesGravitySensitivity(self, posBatch, degree, mu, rE):
        ...
        for kk in range(nBatch):
            ...
            for ii in range(1,degree+2):
                ...
                self.aBar[ii,ii-1] = np.sqrt((2*ii)*getK(ii-1)/getK(ii))*self.aBar[ii,ii]*u
            ...
            for jj in range(0,degree+2):
                for ii in range(jj+2, degree+2):
                    self.aBar[ii,jj] = u*self.n1[ii,jj]*self.aBar[ii-1,jj] - self.n2[ii,jj]*self.aBar[ii-2,jj]
            ...
            for ii in range(2, degree+1):
                ...
                for jj in range(0, ii+1):
                    ...
                    dadCS[3*kk:3*(kk+1),cont_C] = np.array([da0dCij, da1dCij, da2dCij])
                    cont_C += 1
                    if jj > 0:
                        dadCS[3*kk:3*(kk+1),nC+cont_S] = np.array([da0dSij, da1dSij, da2dSij])
                        cont_S += 1
    ...
    def update(self, positions):
        da_dC = self.computePinesGravitySensitivity(positions)
