...
class RLLS:
    ...
    def update(self, rk, yk):
        ...
        Hk = populate_H_singular(rk, self.A, self.n1, self.n2, self.N, self.a, self.mu, self.remove_deg)        
...
def main():
    ...
    C_lm, S_lm = populate_removed_degrees(C_lm, S_lm, sh_EGM2008.C_lm, sh_EGM2008.S_lm, remove_deg)
...
def plot_coef_history(x_hat_hist, P_hat_hist, sh_EGM2008, remove_deg, start_idx=0):
    ...
    for i in range(len(x_hat_hist[0])):
        ...
        plt.plot(x_hat_hist[start_idx:,i] - coef + 3*np.sqrt(P_hat_hist[start_idx:,i]), c='r')
        plt.plot(x_hat_hist[start_idx:,i] - coef - 3*np.sqrt(P_hat_hist[start_idx:,i]), c='r')
