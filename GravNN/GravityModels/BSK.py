...
from GravNN.Trajectories import UniformDist
...
class BSK(AccelerationBase):
    ...
    def generate_full_file_directory(self):
        self.file_directory += (
            os.path.splitext(os.path.basename(__file__))[0] + str(self.errors) + "/"
        )
        pass
    ...
    def compute_acceleration(self):
        ...
        scObject.gravField.gravBodies = spacecraftPlus.GravBodyVector(
            list(bodyInterface.gravFactory.gravBodies.values())
        )

        timeInitString = "2012 MAY 1 00:28:30.0"
        spiceTimeStringFormat = "%Y %B %d %H:%M:%S.%f"
        spiceObject, epochMsg = bodyInterface.gravFactory.createSpiceInterface(
            bskPath + "/supportData/EphemerisData/",
            timeInitString,
            epochInMsgName="simEpoch",
        )
        bodyInterface.configure()
        unitTestSupport.setMessage(
            scSim.TotalSim, simProcessName, spiceObject.epochInMsgName, epochMsg
        )

        scSim.AddModelToTask(
            simTaskName, bodyInterface.gravFactory.spiceObject, None, -1
        )

        pyswice.furnsh_c(
            bodyInterface.gravFactory.spiceObject.SPICEDataPath + "de430.bsp"
        )  # solar system bodies
        pyswice.furnsh_c(
            bodyInterface.gravFactory.spiceObject.SPICEDataPath + "naif0012.tls"
        )  # leap second file
        pyswice.furnsh_c(
            bodyInterface.gravFactory.spiceObject.SPICEDataPath + "de-403-masses.tpc"
        )  # solar system masses
        pyswice.furnsh_c(
            bodyInterface.gravFactory.spiceObject.SPICEDataPath + "pck00010.tpc"
        ...
        posData = scSim.pullMessageLogData(
            scObject.scStateOutMsgName + ".r_BN_N", list(range(3))
        )
        velData = scSim.pullMessageLogData(
            scObject.scStateOutMsgName + ".v_BN_N", list(range(3))
        )
        ...
        rData, vData, accData = computeAcceleration(
            posData_sphere[1:], velData_sphere[1:]
        )
...
def run():
    body = GravBodyInterface(
        "earth",
        "/Users/johnmartin/Basilisk/SupportData/LocalGravData/GGM03S.txt",
        degree=10,
    )
    rIC = (
        UniformDist(body, body.gravBody["earth"].radEquator + 10000, 10000)
        .load()
        .accelerations
    )
