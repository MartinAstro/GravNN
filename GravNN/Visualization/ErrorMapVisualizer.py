...
class ErrorMapVisualizer(MapBase):
    ...
    def plot_models(self, grid_true, grid_pinn, grid_sh):
        ...
        plt.subplot(3,1,1)
        vlim = [grid_pinn.total.min(), np.mean(grid_pinn.total) + 2*np.std(grid_pinn.total)]
    ...
    def plot_percent_error(self, sh_percent, pinn_percent):
        ...
        plt.gcf().get_axes()[-2].set_title(f"PINN Percent Error: {np.average(pinn_percent.total)}")
        
        plt.subplot(2,1,2)
        self.plot_grid(sh_percent.total, vlim=vlim, label=None, new_fig=False)
        plt.gcf().get_axes()[-2].set_title(f"SH Percent Error: {np.average(sh_percent.total)}")

    def plot_rms(self, sh_diff, pinn_diff):
        vlim = [pinn_diff.total.min(), np.mean(pinn_diff.total) + 2*np.std(pinn_diff.total)]
        plt.figure(figsize=(8,12))

        plt.subplot(2,1,1)
        self.plot_grid(pinn_diff.total, vlim=vlim, label=None, new_fig=False)
        plt.gcf().get_axes()[-2].set_title(f"PINN RMS Diff: {np.average(pinn_diff.total)}")
