...
class MapVisualization(VisualizationBase):
    ...
    def add_colorbar(self,im, label, **kwargs):
        ...
        if vlim is not None:
            ...
            cBar = plt.colorbar(im, cax=cax, ticks=t, format=format, extend=extend, orientation=orientation)

        else:
            cBar = plt.colorbar(im, cax=cax, format=format, extend=extend, orientation=orientation)
