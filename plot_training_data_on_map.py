...
from Support.transformations import cart2sph,project_acceleration, check_fix_radial_precision_errors
import os
import pyshtools
...
def plot_grid_on_map():
    ...
    Call_gm = SphericalHarmonics(sh_file, degree=max_deg, trajectory=trajectory_uniform)
    C20_gm = SphericalHarmonics(sh_file, 2, trajectory=trajectory_uniform)
    Call_gm.load()
    C20_gm.load()
    acc = Call_gm.accelerations - C20_gm.accelerations

    pos_sphere = cart2sph(trajectory_uniform.positions)
    pos_sphere = check_fix_radial_precision_errors(pos_sphere)
    acc_proj = project_acceleration(pos_sphere, acc)

    x_train, x_val, y_train, y_val = train_test_split(pos_sphere, acc_proj, random_state=0, test_size=0.3)
