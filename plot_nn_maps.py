...
from run_hyperparameter_inst import compute_error
...
def plot_se_v_coef(trajectory, preprocessor, point_count, vlim=None):
    ...
    ####### Unpack RSE from SH
    with open('./Files/Results/SH_RMSE.data', 'rb') as f:
...
def plot_LEO_se_v_coef(trajectory, preprocessor, point_count, vlim=None):
    ...
    ####### Unpack RSE from SH
    with open('./Files/Results/SH_LEO_RMSE.data', 'rb') as f:
        coefficient_list = pickle.load(f)
        rse_list = pickle.load(f)
        rse_feat_list = pickle.load(f)
...
def main():
    # r = Restore('./hyper_UniformDist_MinMaxTransform_259200_case_6.zip') 
    # model = r.model


    point_count = 259200 # 0.5 Deg
    #point_count = 720*720*2 # 0.5
    planet = Earth()
    #preprocessor = MinMaxTransform()
    #loss_vlim = [0.00060, 0.00110] # For Min Max Transform
    preprocessor = MinMaxStandardTransform()


    #vlim = [0, 40] #    vlim = [0, 0.4] #for LEO
    ...
    #model, history = load_final_model(6, trajectory_train, preprocessor, point_count)
