...
def accuracy():
    ...
    accelerations_x = pines.compute_acceleration(position_x)
    accelerations_y = pines.compute_acceleration(position_y)
    accelerations_z = pines.compute_acceleration(position_z)
    ...
    degree = 4
    pines = PinesAlgorithm(planet.radius, planet.mu, degree,gravityModel.C_lm, gravityModel.S_lm)
    accelerations_all = pines.compute_acceleration([planet.radius, 0, 0, 0, planet.radius, 0, 0, 0, planet.radius])
