...
from GravNN.Regression.utils import compute_A, compute_euler, format_coefficients, getK, RegressSolution
from GravNN.Regression.XuLS import XuLS, AnalyzeRegression
...
def main():
    ...
    x_test, a_test, _ = get_sh_data(test_trajectory,planet.sh_file, max_deg=max_true_deg, deg_removed=-1)
    ...
    # for solver_algorithm in ['least_squares', 'kaula', 'single_parameter', 'xu_rummel_94']:
    for solver_algorithm in ['xu_rummel_94', 'single_parameter']:
        da_vec = []
        dbeta_vec = []
        training_vec = []
        for training_size in [1000, 2500, 5000, 10000]:

            train_trajectory = RandomDist(planet, [planet.radius, planet.radius+420], training_size)
            x, a, _ = get_sh_data(train_trajectory,planet.sh_file, max_deg=max_true_deg, deg_removed=remove_deg)
            ...
            da = np.average(np.linalg.norm(a_pred - a_test,axis=1)/np.linalg.norm(a_test,axis=1)*100)
