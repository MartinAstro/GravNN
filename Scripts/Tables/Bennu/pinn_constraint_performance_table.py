...
def convert_to_formatted_string(df):
    ...
    str_df = pd.DataFrame(data=[[df.loc["N_train"], df.loc['PINN_constraint_fcn'].__name__, interior, exterior]], columns=['N_train', 'Constraint', 'Interior', 'Exterior']).set_index('N_train')
