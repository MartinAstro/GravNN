...
def main():
    ...
    num_units_list = [5, 10]#, 15, 20, 25, 30, 35, 40]
    noise_list =  np.arange(0, 3, 1, dtype=int)
    ...
    for idx in range(num_models):
        ...
        x, a, u = get_sh_data(trajectory, model_file, max_deg=100, deg_removed=2, random_state=idx)
        ...
        for noise in noise_list:
            ...
            for num_units in num_units_list:
                nn_id = regress_nn_model(x, a_biased, x_val, a_val, get_default_earth_config())
                nn_df.loc[(noise, deg, idx)] = nn_id

            # Train on increasingly high capacity PINN
            for num_units in num_units_list:
                pinn_id = regress_nn_model(x, a_biased, x_val, a_val, get_default_earth_pinn_config())
                pinn_df.loc[(noise, deg, idx)] = pinn_id
