...
def main():  
    ...
    for noise in [0.0, 0.2]:
        for max_deg in [4, 8, 16]:
            ...
            file_name = "GravNN/Files/GravityModels/Regressed/Eros/Residual/2R_3R/N_%d_Noise_%.2f.csv" % \
    ...
    for noise in [0.0, 0.2]:
        for max_deg in [4, 8, 16]:
            ...
            dataset, _, _ = get_preprocessed_data(config)
            C_lm, S_lm = compute_coefficients(dataset, max_deg, planet)
            file_name = "GravNN/Files/GravityModels/Regressed/Eros/Residual/0R_3R/N_%d_Noise_%.2f.csv" % \
                            (max_deg, config['acc_noise'][0])
            save(file_name, planet, C_lm, S_lm)
    ...
    for noise in [0.0, 0.2]:
        for max_deg in [4, 8, 16]:
            ...
            dataset, _, _ = get_preprocessed_data(config)
            C_lm, S_lm = compute_coefficients(dataset, max_deg, planet)
            file_name = "GravNN/Files/GravityModels/Regressed/Eros/Residual/2R_3R_Plus/N_%d_Noise_%.2f.csv" % \
                            (max_deg, config['acc_noise'][0])
            save(file_name, planet, C_lm, S_lm)
