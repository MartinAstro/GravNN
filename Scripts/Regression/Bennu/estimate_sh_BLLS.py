            ...
            for max_deg in [4, 8, 16]:
                for remove_deg in [-1, 0]:
                    ...
                    regressor = BLLS(
                        max_deg=max_deg,
                        planet=planet,
                        remove_deg=remove_deg,
                    )
                    results = regressor.update(x_train, a_train, iterations=10)
                    C_lm, S_lm = format_coefficients(
                        results,
                        regressor.N,
                        regressor.remove_deg,
                    )
                    file_name = (
                        "GravNN/Files/GravityModels/Regressed/Bennu/%s/BLLS/N_%d/M_%d/%d/regressed_%.2f.csv"
                        % (
                            config["distribution"][0].__name__,
                            max_deg,
                            remove_deg,
                            config["N_train"][0],
                            config["acc_noise"][0],
