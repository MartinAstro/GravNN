...
def evaluate_network_error(
    ...
    config, model = load_config_and_model(df, ids[-1])
...
def evaluate_nn(trajectory, model_file, models):
    ...
    for model in models:
        if (
            "nn_estimate_r_inner_600.data" in model
            or "nn_estimate_r_outer_600.data" in model
            or "nn_estimate_r_surface_600.data" in model
        ):
            continue
    ...
    for seed in range(0, 5):
        for hopper in [True]:  # , False]:
            for constraint in ["pinn_alc"]:  # ['pinn_a', 'pinn_alc']:
                ...
                model_directory = (
                    os.path.curdir
                    + "/GravNN/Files/GravityModels/Regressed/%s/%s/%s/%s/%s/%s/"
                    % (
                ...
                sample_list, error_list = evaluate_nn(
                    trajectory,
                    planet.stl_200k,
                    models,
                )
                ...
                with open(
                    model_directory + "Data/nn_estimate_" + dist_name + ".data",
                    "wb",
                ) as f:
