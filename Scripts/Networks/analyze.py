...
def main():
    ...
    for id_value in ids:
        ...
        model_id = id_value
        # df_file = 'temp.data'
        # model_id = 2459202.3816087963


        alt_df_file = os.path.abspath('.') +"/Plots/"+str(model_id)+"/rse_alt.data"
        ...
        density_deg = 180# 180
        test_trajectories = {
            "Brillouin" : DHGridDist(earth, earth.radius, degree=density_deg),
            #"LEO" : DHGridDist(earth, earth.radius+420000.0, degree=density_deg),
            #"GEO" : DHGridDist(earth, earth.radius+35786000.0, degree=density_deg)
        }
        rse_entries = analyzer.compute_rse_stats(test_trajectories)
        utils.update_df_row(model_id, df_file, rse_entries)
        #alt_df = analyzer.compute_alt_stats(earth, density_deg)
