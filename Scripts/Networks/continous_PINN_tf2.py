...
def main():
    ...
    config_3.update({'layers' : [[3, 80, 80, 80, 80, 80, 80, 80, 80, 3]],
                     'batch_size' : [40000],
                     'optimizer' : [tf.keras.optimizers.SGD],
                     'lr_scheduler' : [tf.keras.optimizers.schedules.ExponentialDecay(0.01, decay_steps=10000, decay_rate=0.80, staircase=False)],
                    })

    config_4 = copy.deepcopy(config)
    config_4.update({'layers' : [[3, 80, 80, 80, 80, 80, 80, 80, 80, 3]],
                     'batch_size' : [40000],
                     'optimizer' : [tf.keras.optimizers.RMSprop],
                     #'lr_scheduler' : [tf.keras.optimizers.schedules.ExponentialDecay(0.05, decay_steps=10000, decay_rate=0.90, staircase=False)],
                    })
    ...
    for key, config in configurations.items():
        ...
        trajectory = config['distribution'][0](planet, [config['radius_min'][0], config['radius_max'][0]], points=259200)

        Call_r0_gm = SphericalHarmonics(gravity_file, degree=int(config['max_deg'][0]), trajectory=trajectory)
        accelerations = Call_r0_gm.load()

        Clm_r0_gm = SphericalHarmonics(gravity_file, degree=int(config['deg_removed'][0]), trajectory=trajectory)
        ...
        # Initial Data
        idx_train_x = np.random.choice(x.shape[0], config['N_train'][0], replace=False) 
        idx_val_x = np.random.choice(x.shape[0], config['N_val'][0], replace=False) 

        x_train = x[idx_train_x] # (x,y,z) or (r, theta, phi)
        a_train = a[idx_train_x] # (a_x, a_y, a_z) or (a_r, a_theta, a_phi)

        x_val = x[idx_val_x] # (x,y,z) or (r, theta, phi)
        a_val = a[idx_val_x] # (a_x, a_y, a_z) or (a_r, a_theta, a_phi)
