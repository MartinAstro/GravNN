...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, x1, x2, a0, a1, a2, layers, dt, q):
        ...
        self.dummy_y0_tf = tf.placeholder(tf.float32, shape=(None, self.q)) # dummy variable for fwd_gradients
        self.dummy_y1_tf = tf.placeholder(tf.float32, shape=(None, self.q+1)) # dummy variable for fwd_gradients

        self.dummy_z0_tf = tf.placeholder(tf.float32, shape=(None, self.q)) # dummy variable for fwd_gradients
        self.dummy_z1_tf = tf.placeholder(tf.float32, shape=(None, self.q+1)) # dummy variable for fwd_gradients

        # self.U0_pred = self.net_U0(self.x0_tf) # N x (q+1)
        # self.U1_pred, self.U1_x_pred= self.net_U1(self.x1_tf) # N1 x (q+1)
        
        # self.loss = tf.reduce_sum(tf.square(self.u0_tf - self.U0_pred)) + \ # Value of U at 
        #             tf.reduce_sum(tf.square(self.U1_pred[0,:] - self.U1_pred[1,:])) + \
        #             tf.reduce_sum(tf.square(self.U1_x_pred[0,:] - self.U1_x_pred[1,:]))                     
        
        self.loss = tf.reduce_sum(tf.square(self.x0_tf - self.x0_pred)) + \
                    tf.reduce_sum(tf.square(self.x1_tf - self.x1_pred)) + \
                    tf.reduce_sum(tf.square(self.x2_tf - self.x2_pred)) + \
                    tf.reduce_sum(tf.square(self.U1_pred[0,:] - self.U1_pred[1,:])) + \
                    tf.reduce_sum(tf.square(self.U1_x_pred[0,:] - self.U1_x_pred[1,:]))     
...
if __name__ == "__main__": 
    ...
    model = PhysicsInformedNN(x0, v0, a0, layers, dt, q)
    model.train(10000)
