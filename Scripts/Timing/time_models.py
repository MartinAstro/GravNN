...
def time_networks(batch, points):
    ...
    for i, model_id in enumerate(df["id"].values):
        ...
        pinn_df_i = pd.DataFrame(
            {"params": params, "time [s]": np.average(times)},
            index=[i],
        )
...
def main():
    ...
    args = (
        (True, 10000, True),
        (False, 10000, True)(True, 10000, False),
        (False, 10000, False),
    )
    for batch, points, jit in zip(args):
        ...
        pd.to_pickle(
            pinn_df,
            f"Data/Dataframes/pinn_model_times_{batch}_{points}_{jit}.data",
        )

        sh_df = time_spherical_harmonics(batch=batch, points=points, jit=jit)
        pd.to_pickle(
            sh_df,
            f"Data/Dataframes/sh_model_times_{batch}_{points}_{jit}.data",
        )
