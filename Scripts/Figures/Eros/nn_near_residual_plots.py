...
def overlay_hist(x_sph_train,twinx=True):
    if twinx:
        plt.gca().twinx()
    plt.hist(x_sph_train[:,0], bins=100,alpha=0.7,  range=[np.min(x_sph_train[:,0]), np.max(x_sph_train[:,0])], color='gray')#edgecolor='black', linewidth=0.5, fill=True)
...
def main():
    ...
    df, descriptor = pd.read_pickle("Data/Dataframes/near_all_data.data"), "[5000,10000]"
