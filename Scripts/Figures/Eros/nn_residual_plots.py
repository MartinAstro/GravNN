...
def plot_moving_average(x, y, y_pred, percent=True, alpha=0.5,label=None, marker=None, color='black'):
    if not percent:
        diff = y - y_pred
    else:
...
def main():
    ...
    for model_id in df['id'].values[:]:
        ...
        plt.figure(1)
        data_vis.plot_residuals(x_sph[:,0], u, u_pred[:,0], alpha=0.5, label=label, plot_truth=plot_truth, ylabel='Potential')
        plot_moving_average(x_sph[:,0], u, u_pred[:,0], marker=marker_list[q], color=color_list[q])
        # plt.gca().yaxis.set_ticks_position('left')
        # plt.gca().yaxis.set_label_position('left')

        plt.figure(2)
        data_vis.plot_residuals(x_sph[:,0], a_sph[:,0], a_sph_pred[:,0], alpha=0.5,label=label, plot_truth=plot_truth, ylabel='Acceleration')
        plot_moving_average(x_sph[:,0], a_sph[:,0], a_sph_pred[:,0], marker=marker_list[q], color=color_list[q])
