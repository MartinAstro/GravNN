...
def main():
    ...
    C22_r0_gm = SphericalHarmonics(sh_file, degree=2, trajectory=trajectory).load()
    C22_p = format_potential_as_Nx3(C22_r0_gm)

    grid_true_potential = Grid(trajectory=trajectory, accelerations=Call_p - C22_p)
    grid_true_accelerations = Grid(
        trajectory=trajectory,
        accelerations=Call_r0_gm.accelerations - C22_r0_gm.accelerations,
    )
