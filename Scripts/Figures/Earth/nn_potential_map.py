...
def get_title(config):
    title = (
        str(config["PINN_constraint_fcn"][0])
        + "-"
...
def plot_potential_full_C22(map_vis, grid_pot_true, grid_C22):
    ...
    plt.subplot(2, 1, 2)
    map_vis.plot_grid(
        (grid_pot_true - grid_C22).total,
        "Potential C22 Removed",
        new_fig=False,
    )
...
def true_pred_acc_pot_plots(
    ...
    map_vis.plot_grid(
        ...
        new_fig=False,
        ticks=False,
    )  # , vlim=[ax.images[0].colorbar.vmin, ax.images[0].colorbar.vmax])
    plt.subplot(2, 3, 3)
    map_vis.plot_grid(
        (grid_pot_true - grid_pot_pred).total,
        "Pot Diff",
        new_fig=False,
        ticks=False,
    )  # , vlim=[0,10000])
    ...
    plt.subplot(2, 3, 6)
    map_vis.plot_grid(
        (grid_acc_true - grid_acc_pred).total,
        "Acc Diff",
        new_fig=False,
        ticks=False,
        vlim=[0, 0.025],
...
def main():
    ...
    RandomDist(
        planet,
        [radius_min, planet.radius + 420000],
        points=20000,
    )
    ...
    grid_pot_true = Grid(
        trajectory=DH_trajectory,
        accelerations=u_3vec,
        transform=False,
    )
    grid_acc_true = Grid(
        trajectory=DH_trajectory,
        accelerations=Call_r0_gm.accelerations,
    )

    u_3vec[:, 0] = C22_r0_gm.potentials
    Grid(trajectory=DH_trajectory, accelerations=u_3vec)
    ...
    for model_id in model_ids:
        ...
        data_vis.plot_acceleration_dist(
            x_sph,
            a_sph,
            a_sph_pred,
            "Spherical Acceleration Error",
        )

        grid_pot_pred = Grid(
            trajectory=DH_trajectory,
            accelerations=format_potential_as_Nx3(u),
            transform=False,
        )
        grid_acc_pred = Grid(trajectory=DH_trajectory, accelerations=a_pred)

        true_pred_acc_pot_plots(
            map_vis,
            grid_pot_true,
            grid_pot_pred,
            grid_acc_true,
