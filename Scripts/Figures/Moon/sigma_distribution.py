...
def std_masks(grid, sigma):
    sigma_mask = np.where(
        grid.total > (np.mean(grid.total) + sigma * np.std(grid.total)),
    )
...
def cumulative_distribution():
    ...
    total = len(sorted_data) - len(
        np.where(
            grid_Call_m_C22.total
            > (np.mean(grid_Call_m_C22.total) + 1 * np.std(grid_Call_m_C22.total)),
        )[0],
    )
    total2 = len(sorted_data) - len(
        np.where(
            grid_Call_m_C22.total
            > (np.mean(grid_Call_m_C22.total) + 2 * np.std(grid_Call_m_C22.total)),
        )[0],
    )
    total3 = len(sorted_data) - len(
        np.where(
            grid_Call_m_C22.total
            > (np.mean(grid_Call_m_C22.total) + 3 * np.std(grid_Call_m_C22.total)),
        )[0],
    )
...
def acceleration_masks():
    ...
    grid_Call_m_C22 = grid_true - grid_C22
    sigma_5_mask, sigma_5_mask_compliment = std_masks(grid_Call_m_C22, 5)
    sigma_4_mask, sigma_4_mask_compliment = std_masks(grid_Call_m_C22, 4)
    sigma_3_mask, sigma_3_mask_compliment = std_masks(grid_Call_m_C22, 3)
    sigma_2_mask, sigma_2_mask_compliment = std_masks(grid_Call_m_C22, 2)
    sigma_1_mask, sigma_1_mask_compliment = std_masks(grid_Call_m_C22, 1)

    # plt.figure()
    # plt.hist(grid_Call_m_C22.total[sigma_5_mask].reshape((-1,)), 100)
    # plt.hist(grid_Call_m_C22.total[sigma_5_mask_compliment].reshape((-1,)), 100)
    ...
    layer_grid.total[sigma_1_mask_compliment] = 0.0
    layer_grid.total[sigma_1_mask] = 1.0
    layer_grid.total[sigma_2_mask] = 2.0
    layer_grid.total[sigma_3_mask] = 3.0
    layer_grid.total[sigma_4_mask] = 4.0
    layer_grid.total[sigma_5_mask] = 5.0
    ...
    # data = grid_Call_m_C22.total
    # directory = os.path.abspath('.') +"/Plots/"
    # fig, ax = map_vis.plot_grid(data, "Truth", vlim)
    # #map_vis.save(fig, directory + 'OneOff/true_' +  + '.pdf')

    # sigma_1_values = data[sigma_1_mask]
    # data[sigma_1_mask] = 0.0
    # fig, ax = map_vis.plot_grid(data, "1-Sigma Feature Compliment", vlim)
    # #map_vis.save(fig, directory + 'OneOff/sigma_1_mask_compliment_' +  + '.pdf')
    # data[sigma_1_mask] = sigma_1_values

    # sigma_1_compliment_values = data[sigma_1_mask_compliment]
    # data[sigma_1_mask_compliment] = 0.0
    # fig, ax = map_vis.plot_grid(data, "1-Sigma Features", vlim)
    # #map_vis.save(fig, directory + 'OneOff/sigma_1_mask_' +  + '.pdf')
    # data[sigma_1_mask_compliment] = sigma_1_compliment_values

    # sigma_2_values = data[sigma_2_mask]
    # data[sigma_2_mask] = 0.0
    # fig, ax = map_vis.plot_grid(data, "2-Sigma Feature Compliment", vlim)
    # #map_vis.save(fig, directory + 'OneOff/sigma_2_mask_compliment_' +  + '.pdf')
    # data[sigma_2_mask] = sigma_2_values

    # sigma_2_compliment_values = data[sigma_2_mask_compliment]
    # data[sigma_2_mask_compliment] = 0.0
    # fig, ax = map_vis.plot_grid(data, "2-Sigma Features", vlim)
    # #map_vis.save(fig, directory + 'OneOff/sigma_2_mask_' +  + '.pdf')
    # data[sigma_2_mask_compliment] = sigma_2_compliment_values

    # sigma_3_values = data[sigma_3_mask]
    # data[sigma_3_mask] = 0.0
    # fig, ax = map_vis.plot_grid(data, "3-Sigma Feature Compliment", vlim)
    # #map_vis.save(fig, directory + 'OneOff/sigma_3_mask_compliment_' +  + '.pdf')
    # data[sigma_3_mask] = sigma_3_values

    # sigma_3_compliment_values = data[sigma_3_mask_compliment]
    # data[sigma_3_mask_compliment] = 0.0
    # fig, ax = map_vis.plot_grid(data, "3-Sigma Features", vlim)
    # #map_vis.save(fig, directory + 'OneOff/sigma_3_mask_' +  + '.pdf')
    # data[sigma_3_mask_compliment] = sigma_3_compliment_values
